 ...
 * @param {string} [options.libDir] Shared modules served under /lib/ (.mjs only).
 * @param {typeof fetch} [options.fetchImpl]
 * @param {number} [options.maxBodyBytes]
 */
export function createApp({ apiKey, publicDir, libDir, fetchImpl = fetch, maxBodyBytes = 1_000_000 }) {
  const root = path.resolve(publicDir);
  const libRoot = libDir ? path.resolve(libDir) : null;
  ...
  async function serveStatic(res, pathname) {
    ...
    let base = root;
    const isLib = libRoot !== null && rel.startsWith("/lib/");
    if (isLib) {
      base = libRoot;
    ...
    if (isLib && path.extname(resolved) !== ".mjs") {
