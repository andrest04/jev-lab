...
export function validateRequest(request) {
  ...
  if (isEmptyInput(request.state)) {
    fail("state", "State is required. Give the model the text or JSON it should judge.");
  }
  if (typeof request.model !== "string" || request.model.trim() === "") {
    fail("model", "Model is required, for example jev-latest.");
  }

  const questions = request.questions;
  if (!isPlainObject(questions) || Object.keys(questions).length === 0) {
    fail("questions", "Add at least one question.");
  ...
  for (const [id, q] of Object.entries(questions)) {
    ...
    if (!isPlainObject(q)) {
      fail(`questions.${id}`, "A question must be an object.");
      continue;
    }
    if (!QUESTION_TYPES.includes(q.type)) {
      fail(at("type"), `Type must be one of ${QUESTION_TYPES.join(", ")}.`);
      continue;
    }
    if (isEmptyInput(q.instructions)) {
      fail(at("instructions"), "Instructions are required. State the exact condition to judge.");
    }
    ...
    if (q.type === "noul" && q.criteria !== undefined && !isPlainObject(q.criteria)) {
      fail(at("criteria"), 'Noul criteria must be an object with "true" and "false" keys.');
    }
  }
...
function validateChoice(q, at, fail) {
  if (!isPlainObject(q.criteria)) {
    fail(at("criteria"), "A choice needs criteria: an object of option names.");
    return;
  }
  const count = Object.keys(q.criteria).length;
  if (count < LIMITS.minChoiceOptions) {
    fail(at("criteria"), `A choice needs at least ${LIMITS.minChoiceOptions} options.`);
  }
  if (count > LIMITS.maxChoiceOptions) {
    fail(at("criteria"), `A choice allows at most ${LIMITS.maxChoiceOptions} options.`);
  }
}

function validateScore(q, at, fail) {
  if (!Array.isArray(q.criteria)) {
    fail(at("criteria"), "A score needs criteria: an ordered list of level descriptions.");
    return;
  }
